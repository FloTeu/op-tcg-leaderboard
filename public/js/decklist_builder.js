...
(function () {
  ...
  window._cdbLeaderChange = function (sel) {
    ...
    if (window._cdb) {
      window._cdb.setLeader(opt.value, opt.dataset.leaderName || opt.value, opt.dataset.leaderImg || '');
    }
  };
  ...
  window._cdbSetup = function () {
    var init = window._cdbInit || {
      cards: {}, leaderId: '', leaderName: '', leaderImg: '', customId: null
    };
    ...
    window._cdb = {
      ...
      setLeader: function (id, name, img) {
        var cdb = this;
      ...
      renderLeader: function () {
        ...
        if (this.leaderId) {
          ...
          el.innerHTML =
            '<span class="text-gray-400 text-sm">No leader selected — search and click Set Leader</span>';
        }
      },
        ...
        Object.entries(cards).forEach(function (entry) {
          ...
          row.innerHTML =
            '<div class="flex items-center flex-1 min-w-0 mr-2">' +
              ...
              '<span class="text-white text-xs truncate">' + data.name + '</span>' +
