...
from op_tcg.backend.models.cards import LimitlessCardData, CardReleaseSet
...
from op_tcg.backend.crawling.items import TournamentItem, CardPricesItem, ReleaseSetItem, CardsItem, OpTopDecksItem, SealedProductItem
...
class CardPricePipeline:
    ...
    def process_item(self, item: CardPricesItem, spider):
        """
        Loads card price data to BigQuery
        """
        if isinstance(item, CardPricesItem):
            for price in item.prices:
                if price.card_id not in spider.price_count:
                    spider.price_count[price.card_id] = {}
                spider.price_count[price.card_id][price.aa_version] = spider.price_count[price.card_id].get(price.aa_version, 0) + 1

            bq_insert_rows([json.loads(price.model_dump_json()) for price in item.prices],
                           table=spider.price_table, client=spider.bq_client)
        return item
