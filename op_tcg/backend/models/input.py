...
class MetaFormat(EnumBase, StrEnum):
    ...
    def to_list(cls, only_after_release: bool = True, until_meta_format: str | None = None,
                region: MetaFormatRegion = MetaFormatRegion.ALL) -> list[str]:
        """
    ...
    @classmethod
    def latest_meta_format(cls, only_after_release: bool = True,
                           region: MetaFormatRegion = MetaFormatRegion.ALL) -> "MetaFormat":
...
def meta_format2release_datetime(meta_format: MetaFormat,
                                 region: MetaFormatRegion = MetaFormatRegion.WEST) -> datetime | None:
    """
    ...
    western_releases = {
        ...
        MetaFormat.OP12: datetime(2025, 8, 22),
...
def meta_format2side_meta_format(meta_format: MetaFormat,
                                 region: MetaFormatRegion = MetaFormatRegion.WEST) -> SideMetaFormat | None:
    """
...
def get_meta_format_by_datetime(dt: datetime, region: MetaFormatRegion = MetaFormatRegion.WEST) -> MetaFormat:
    ...
    for meta_format in sorted(MetaFormat.to_list(only_after_release=True, region=MetaFormatRegion.ALL), reverse=True):
